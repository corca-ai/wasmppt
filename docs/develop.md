...
fresh Rust 1.88.0 build. A Rust runtime change must therefore commit the regenerated browser bytes
produced by `RUSTUP_TOOLCHAIN=1.88.0 npm run build:wasm-hosts`; a Git dependency cannot silently
ship an older embedded engine. This release-artifact compiler matches the optional metafile MSRV
and the performance baseline; the pinned development compiler remains authoritative for ordinary
local checks.
